...
statement_path = r"C:\Users\Mike\OneDrive - The Pennsylvania State University\Finances\2021 Taxes\2021 Accounting\Amex Statements"

# Setup excel sheet
# create excel sheet to output information of each receipt
workbook = xlsxwriter.Workbook(statement_path+r'\AmexExpenses.xlsx')
...
for entry in os.scandir(statement_path):
    ...
    # Looking for: "AccountSummaryMICHAELAPAGANClosingDate03/08/21AccountEnding1-81001NewBalance$88.33"
    ...
    # Extract the date
    Period_wild = re.compile("ClosingDate\d{2}\/\d{2}\/\d{2}")
    Period_search = re.search(Period_wild, statement_text)
    Statement_period = Period_search.group()

    # Extract the dates
    Statement_period = Statement_period.replace("ClosingDate","")
